...
barplot(ethnicity, cex.names = 0.5, beside=TRUE, main="Figure 1. Respondents Voting Habits By Ethnicity", xlab= "Ethnicity", col = c("red", "blue"))
legend("topleft", legend= c("Donald Trump", "Joe Biden"), fill = c("red", "blue"))
```

Ethnic minorities, as shown by the Nationscape data, tend to favour the democratic candidate, Joe Biden. White voters on the other hand, appear to be planning their vote for Trump
...
col = c("red","blue")
barplot(gen_vote, names.arg=c("Donald Trump", "Joe Biden"), beside = TRUE, col = col, border = "white", main="Figure 2. How Respondents Plan to Vote in 2020 By Gender", ylab="Number of respondents", xlab="Options for 2020 American Election", cex.names = 0.7)
legend("top", legend = c("Female", "Male"), fill=col, cex = 0.7)
```
...
Voters younger than 35 years of age strongly prefer Joe Biden. Respondents over this age seem to slightly prefer Donald Trump but some age groups are too close to call. 

```{r partyassoc, echo=FALSE}
vote_D <-filter(reduced_data, vote_2020=="0")
ideo <-table(vote_D$ideo5)
pie(ideo, main="Figure 4. Political Party Association of Respondents Voting for Donald Trump", col = hcl.colors(6, "Reds 3"), cex.main = 0.85)
vote_J <-filter(reduced_data, vote_2020=="1")
ideoJ <-table(vote_J$ideo5)

pie(ideoJ, main=" Figure 5. Political Party Association of Respondents Voting for Joe Biden",  col = hcl.colors(6, "Blues 3"),  cex.main = 0.85)
...
```{r CensusAnalysis, echo=FALSE, warning=FALSE}
...
census_data %>% 
  ...
  labs(x="Gender",
       ...
       title="Figure 9. Gender Proportion in the American Community Survey (2018) data")
```

To show that it is appropriate to use the 2018 ACS data for post stratification, the variables used in the model must be in proportions representative of the American population. Since there are slightly more females in the general American population, it is suitable that the proportion of females is a bit greater in the ACS. 

```{r CensusAnalysis age, echo=FALSE, warning=FALSE}
suppressMessages(
census_data %>% 
  ggplot(aes(x=age)) +
  geom_bar(fill='red', color='black')+
  ...
  labs(x="Age range",
       y="Number of Respondents",
       title="Figure 10. Age range in the US Estimated Through American Community Survey (2018)"))
```

Aligned with population data, a slight left skew is demonstrated by the age group data in the2018 ACS survey. 

```{r CensusAnalysis employment, echo=FALSE, warning=FALSE}
suppressMessages(pie(table(census_data$employment), main = "Figure 11. American Community Survey (2018) Respondents Employment Status", cex.main = 0.8, col = hcl.colors(3, "PuBu")))
```

Since the majority of the American public was employed at the time of the 2018 ACS survey, these results appear fitting to simulate our presidential prediction model. However, this could be a source of error as the unemployment rate is currently higher due to the pandemic. 
...
State-by-state responses are visualized to show where the highest number of respondents resides. California, Texas, Florida and states closer to the coasts tend to have the most respondents in the ACS survey. Since California, Texas and Florida are the most populated states (in that order); it is good news as a base for our simulation. 

```{r income_distribution_by_state, echo=FALSE}
#Census data
par(las=2)
GraphIndex <- c("Very High Income", "High Income", "Low Income", "Lower Middle Income", "Under Poverty Line", "Upper Middle Income")

barplot(table(census_data$household_income, census_data$state), cex.names = 0.5, col = hcl.colors(6, "Blues"), main ="Figure 13: Estimates of Income Stratification By State")
legend("topright", 
       legend = GraphIndex, 
       fill = hcl.colors(6, "Blues"), ncol = 2,
       cex = 0.75)
```

Income was another key variable in our simulation model and it can be shown here that states vary by proportion of respondents at various income levels. 

```{r Race_By_State,  echo=FALSE, warning=FALSE}
#Census Data
GraphIndex <- c("Asian/Pacific Islander", "Black", "Native", "Other/Mixed", "White")
suppressMessages(par(las=2))
suppressMessages(
barplot(table(census_data$race_ethnicity,  census_data$state), cex.names = 0.5, col = hcl.colors(5, "Reds 3"), main ="Figure 14: Distribution of Race Ethnicities By State"))
legend("topright", 
       legend = GraphIndex, 
       fill = hcl.colors(5, "Reds 3"), ncol = 1,
       cex = 0.75)
```

Distribution of race ethnicities throughout America are shown here to be more common in the southern states as well as states near the Pacific Ocean (excluding Alaska)
...
Proportion of Hispanic residents in the 2018 ACS is distributed throughout the southern United States and states along the coast, similar to race ethnicities. 
...
We took income and age which were continuous variables in the survey and census and grouped them into income and age groups that we could poststratify more easily. These groups are also done in such a way that they represent voting blocs in general. 

A similar process was done with the race and ethnicity data to aggregate smaller racial groups into broader ones that could be matched easily between the survey and census. The variable Hispanic was also aggregated into a binary variable as opposed to a more specific country specific categorical variable.

In our analysis, gender and sex were made equivalent since there was an absence of other options of gender identity and we did not have any meaningful demographic information on non-binary respondents. Employment was also compressed from the survey data to match broad categories of employed, unemployed, and out of labour force. This captured the majority of the nuance between employment status that we were looking to examine in this analysis.
...
For a logistic model to be valid, we also need certain conditions to be true with our data. This includes the independence of each observation and that there is low covariance between any pair of explanatory variables.
...
One limitation of our model is that it was restricted to only one phase of the Nationscape Data. Although 5200 respondents is a good size survey, it may not properly model a population of 350 million. For example, current literature has shown that well educated citizens, those holding degrees higher than a Bachelor’s, identify as liberal more than republican. However, the data set used to model our prediction did not show a significant difference in voter ideologies based on education level.

A second issue with the source data for the model is that it is limited to those who are adept at technology. While many younger people would be confident taking the survey, the older generations are the most reliable voters. Older voters who are less comfortable with technology may be underrepresented possibly biasing the results. ( this is fixed by post-stratification tho?)  

A third limitation of our model is the lack of segregating Nebraska and Maine into congressional districts for predictions of plurality. The other forty-eight states of the US use plurality to decide the electors, however in Nebraska and Maine only two electors are decided this way and the rest are decided based on plurality in each congressional district. This is a minor issue however, as the congressional districts should each follow the plurality of the state and the two states only have 9 of 538 electoral votes. 

The 2020 U.S. election is also particularly unique because it is the first federal election for which a majority of ballots will be cast via mail-in voting. The effects of this on voter turnout were not taken into account as a factor in our model. 
 
Another limitation with our predictions is that the survey we use for our model is from June. Recent developments that could swing voter preference are not accounted for in our model and represent a possible bias that can affect the credibility of our predictions. For instance Trump’s recent management of the Covid-19 pandemic and the effect it’s taken on the economy has experts believing his approval rating is currently declining (The Economist 2020). After contracting Covid-19 himself, many believe Trump will not be able to fulfill his duties as president and cannot handle managing the pandemic response. Overall, current news regarding Trump can cause a bias in our data set of inflating the number of Trump voters. Particularly older age demographics that are concerned the most with Covid-19 will see the most inflation. This will affect not only the model’s voting percentage for Trump, but also the relationship between age group and vote. This is the most concerning due to older age demographics typically having a higher proportion of Republican voters (Desilver 2014).


## Future Work
	
Future studies for the next election should primarily study voting preferences of swing states in the US. Since the elections work via plurality of each state rather than the country as a whole, studies should first determine swing states via a quick survey. States with less than a ten percent perceived difference between the two leading parties should be considered for a further study to predict the election winner. This method will be more time and cost-effective than conducting surveys in states where there is a clear frontrunner.

Mail-in voting also provides an additional avenue for future work as mail-in ballots will be used in the 2020 election on a wider scale than any previous U.S. election. An analysis on the habits of voters who voted using a mail-in ballot warrants further investigation.

Finally our model offers the Republican and Democratic Parties future targets to focus their campaigns towards in the next election. They can view states that are close in votership but are favoured towards their opposition and collect more data on the demographics of the state and policies people in the state favour. The parties can also look at the average demographics that support their opponent using our model and survey these specific demographics for changes they’d like to see in their party to increase votership.

## Appendix

The code, scripts, and RMarkdown file in this report's analysis can also be found at the following link: https://github.com/brdiep113/forecasting_us_election
...
Flood, S., Goeken, R., Grover, J.,  Meyer, E., Pacas, J., & Sobek, M (2020). IPUMS USA: Version 10.0 [American Community Survey 2018]. Minneapolis, MN: IPUMS. Retrieved from: https://doi.org/10.18128/D010.V10.0.
