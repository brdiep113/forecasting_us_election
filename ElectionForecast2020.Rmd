...
table <- c(1.21,
           ...
           0.42,
           0.),
              round(summary(all_lm)$coefficients[2], digits = 4),
              round(sigma(all_lm) ** 2, digits = 4),
              round(summary(all_lm)$coefficients[,4][2], digits=4),
              round(confint(all_lm, 'list', level=0.95)[1], digits = 4),
              round(confint(all_lm, 'list', level=0.95)[2], digits = 4))

table_titles <- c("Intercept", "Gender<br/>Male", "Estimated<br/>Slope", "Estimated Variance<br/>Of Errors", "p-value for Slope Null<br/>Hypothesis Test", "Slope CI<br/>Lower Bound", "Slope CI<br/>Upper Bound")

table_rows <- c("Bayesian Model")
colnames(table) <- table_titles
rownames(table) <- table_rows


kbl(table, escape=FALSE) %>%
